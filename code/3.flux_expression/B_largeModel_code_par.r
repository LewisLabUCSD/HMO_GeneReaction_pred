...
# 	gc=na.omit(gc)
	...
	ggsave(plot=g,filename='figures/gene_expression.pdf',width=20)
	...
	ggsave(plot=g2,filename='figures/gene_expression.bytime.pdf',height=10,width=20)
	...
	ggsave(plot=g3,filename='figures/gene_expression.bysubject.pdf',height=10,width=20)
...
# load scores codes
combinations = do.call(rbind,strsplit(gsub('_sec','-sec',system('ls data/scores_split/*.mat',intern=T)),'_|\\.'))
unq_tmp=unique(paste(combinations[,4],ifelse(combinations[,5]=='mat',NA,combinations[,5]),sep='_'))
tmp=do.call(rbind,strsplit(unq_tmp,'_'))

# load model codes
combinations2 = do.call(rbind,strsplit(gsub('_sec','-sec',system('ls data/models_split/*.mat',intern=T)),'_|\\.'))
unq_tmp2=unique(paste(combinations2[,3],ifelse(combinations2[,4]=='mat',NA,combinations2[,4]),sep='_'))
...
old_fuc_vec = c('POFUT1b','POFUT2a','POFUT2b','FUT11','FUT2','FUT3','FUT4','FUT6b','FUT8')
...
save(combinations_unq,stat,gl_names,linkage,which.max.na,max.na,mean.na,DS,linkage_orig,gl_names_orig,old_fuc_vec,file='combinations_unq.rda')
...
Gene_Linkage_Scores = lapply(DS,function(dataset){
	...
	breaks<-unlist(foreach(cbx = combinations_unq,.errorhandling='pass',.packages='R.matlab') %dopar% {
		...
		dim( readMat(paste0('data/scores_split/',stat,'_',dataset,'_',cbx,'.mat'))[[1]] )[2]
...
#	desc = describe(gene_linkage_raw)
	...
	#for(ci in 1:length(combinations_unq)){
		...
		r=readMat(paste0('data/scores_split/',stat,'_',dataset,'_',cbx,'.mat'))
		r=r[c(2:10,1)]
		# clean fut
		#for(futi in c(2,3,9)){ r[[futi]] = r[[futi]][old_fuc_vec %in% linkage[[futi]], ]}

		if( any( sapply(r,nrow) != sapply(linkage_orig,length) )){stop('linkage names dont match raw linkage scores')}
		imp=readMat(paste0('data/scores_split/model_impossible_',cbx,'.mat'))[[1]]
#		j=(i+dim(r[[1]])[2]-1)
		i = break_idx[ci]+1
		j = break_idx[ci+1]
...
tmp<-foreach(ds = DS,.packages=c('bigmemory')) %dopar% {
	...
	#z = scale(score,center=median(score))
	z = scale(score)
...
pdf('figures/sigmoid_models.pdf',height=10,width=20)
	score = Model_Scores[,DS=='data1']
	score2= Model_Scores[,DS== 'data2' ]
	lab = Model_Score_Groups[,DS=='data1']
	lab2 = Model_Score_Groups[,DS== 'data2']

#	df = data.frame(score=score,lab=ifelse(lab&lab2,3,ifelse(lab,1,ifelse(lab2,2,0))))
	df = data.frame(score=score,lab=factor(ifelse(lab&lab2,'Commonly Selected',ifelse(lab,'Cohort1 Selected','Not Selected'))))
	df = df[order(df$score,na.last=T),]
	df$model_rank = 1:nrow(df)


# p <- ggplot(df[sample(1:nrow(df),1e5),], aes(x=model_rank,y=score,color=lab)) + geom_point(size=.5) + theme_classic()
# ggExtra::ggMarginal(p, type = "boxplot",groupColour=T)
# ggExtra::ggMarginal(p, type = "density",groupColour=T)

p<-ggplot(na.omit(df[idx<-sample(1:nrow(df),1e5),]), aes(x=model_rank,y=score,color=lab)) +
	geom_point(size=.5) + theme_classic(base_size=15) + scale_color_manual(values=c('black','red','grey'))
#ggExtra::ggMarginal(p, type = "boxplot",groupColour=T)
g = ggExtra::ggMarginal(p, type = "density",groupColour=F,groupFill=T)
#ggExtra::ggMarginal(p, type = "violin",groupColour=T)
#ggExtra::ggMarginal(p, type = "histogram",groupFill=T)

# mpg
ggqqplot(df$score[idx], ylab = "Cohort1 Score")

	tmp=df
	idx<-sample(1:nrow(df),1e5)
	plot(df$model_rank[idx],df$score[idx],col=c('grey','black','red')[factor(df$lab)])
...
if(run_inclusion){
	...
	inclusion_desc<-foreach(cbx = combinations_unq,.packages=c('R.matlab','bigmemory')) %dopar% {
		require(bigmemory)
		describe( as.big.matrix( readMat(paste0('data/models_split/solCombi_',cbx,'.mat'))$solCombi ,type='integer',
	...
	inclusion = attach.big.matrix( paste0('inclusion.rxn.desc') )
}
...
Structure_Choice = lapply(DS,function(dataset){
	...
	out<<-data.frame(
		...
		background_success=foreach(idx=1:nrow(inclusion),.combine=c,.packages=c('bigmemory')) %dopar% {
			require(bigmemory); sum( attach.big.matrix('inclusion.rxn.desc')[idx,!infeasible] ,na.rm=T) },
		selection_success=foreach(idx=1:nrow(inclusion),.combine=c,.packages=c('bigmemory')) %dopar% {
			require(bigmemory); sum( attach.big.matrix('inclusion.rxn.desc')[idx,selected_models ] ,na.rm=T) },
		selection_inter_success=foreach(idx=1:nrow(inclusion),.combine=c,.packages=c('bigmemory')) %dopar% {
			require(bigmemory); sum( attach.big.matrix('inclusion.rxn.desc')[idx,selected_models_inter] ,na.rm=T) }
	)	
	...
	out$enrichment_inter = phyper( out$selection_inter_success-1 , m = out$background_success , 
		n = out$background_failure , k = out$selection_inter_size ,lower.tail=F)
	...
	out$mean_MS_background = do.call(rbind, tmp<-foreach(idx = 1:nrow(inclusion),.packages=c('bigmemory','broom'))  %dopar% {
		require(bigmemory); load('tmp.rda'); i = attach.big.matrix('inclusion.rxn.desc')[idx,] ; 
			model_score_i=attach.big.matrix('Model_Scores.desc')[,DS==dataset];
		idx_down = sample(length(i),1e4)
		#idx_down = sample(c(TRUE,FALSE),size=round(length(i)/100),prob=c(.09,.91),replace=T) 
		if( length(table( i[idx_down] ))<2 ) {return(rep(NA,4))}
		c( mean0 = (mn0<-mean(model_score_i[i==0][idx_down],na.rm=T)) , SD0=sd(model_score_i[i==0][idx_down],na.rm=T),
			mean1 = (mn1<-mean(model_score_i[i==1][idx_down],na.rm=T)) , SD1=sd(model_score_i[i==1][idx_down],na.rm=T) )
...
#	out$t_test_selected = do.call(rbind, apply(inclusion,1,function(i){
	out$mean_MS_selected = do.call(rbind, foreach(idx = 1:nrow(inclusion), .packages=c('bigmemory','broom'))  %dopar% {
...
#	out$t_test_selected = do.call(rbind, apply(inclusion,1,function(i){
	out$mean_MS_selected_inter = do.call(rbind,foreach(idx = 1:nrow(inclusion),.packages=c('bigmemory','broom'))  %dopar% {
		require(bigmemory); load('tmp.rda'); i = attach.big.matrix('inclusion.rxn.desc')[idx,] ; model_score_i=attach.big.matrix('Model_Scores.desc')[,DS==dataset]; 
...
#if(!any(grepl('mean0',colnames(Structure_Choice[[1]])))){stop("missing expected variable: 'mean0'")}
...
write.xlsx(Structure_Choice,file=paste0('data_out/Structure_Choice.xlsx'))
...
if(run_Struc_CBX){
	...
	inclusion=attach.big.matrix('inclusion.rxn.desc')
	...
	for(d_i in names(datas)){ for(v_i in names(vars)){
		...
		grbs =lapply(strs,function(x){
			...
			data_tmp$enrichment = apply( data_tmp , 1 , function(x){
				if(grepl('inter',x[3])){
					data_tmp_e$value[data_tmp_e$ds==x[1]&data_tmp_e$rxn_name==x[2]&grepl('inter',data_tmp_e$variable)]
				}else{
					data_tmp_e$value[data_tmp_e$ds==x[1]&data_tmp_e$rxn_name==x[2]&!grepl('inter',data_tmp_e$variable)]
				}})
...
#	inclusion = t(inclusion)
	...
	out<<-data.frame(
		...
		background_size= dim(inclusion)[1]-sum(infeasible), 
		selection_size = length( selected_models ),
		selection_inter_size = length( selected_models_inter ),
	...
	print('mean -background')
	#out$t_test_background = do.call(rbind, apply(inclusion,1,function(i){
	out$mean_MS_background = do.call(rbind, foreach(idx = 1:ncol(inclusion),.packages=c('bigmemory','broom'))  %dopar% {
...
#		idx_down = sample(c(TRUE,FALSE),size=round(length(i)/100),prob=c(.09,.91),replace=T) 
	...
	print('mean - selected')
#	out$t_test_selected = do.call(rbind, apply(inclusion,1,function(i){
	out$mean_MS_selected = do.call(rbind, foreach(idx = 1:ncol(inclusion),.packages=c('bigmemory','broom'))  %dopar% {
		require(bigmemory);load('tmp.rda');  i = attach.big.matrix( paste0('best_gene.',dataset,'.desc')  )[,idx] ; 
		model_score_i=attach.big.matrix('Model_Scores.desc')[,DS==dataset]; 
		selected_models=(attach.big.matrix('Model_Score_Groups.desc')[,DS==dataset])==1
		i = i[selected_models]; model_score_i = model_score_i[selected_models]
...
#		tmp=NA; try( tmp<-tidy( t.test( model_score_i[idx_down & i==1] , model_score_i[idx_down & i==0]  ,na.action="na.omit") ) ); tmp
	...
	print('mean - selected_inter')
#	out$t_test_selected = do.call(rbind, apply(inclusion,1,function(i){
	out$mean_MS_selected_inter = do.call(rbind, foreach(idx = 1:ncol(inclusion),.packages=c('bigmemory','broom'))  %dopar% {
		require(bigmemory);load('tmp.rda');  i = attach.big.matrix( paste0('best_gene.',dataset,'.desc')  )[,idx] ; 
		model_score_i=attach.big.matrix('Model_Scores.desc')[,DS==dataset]; 
		selected_models=(attach.big.matrix('Model_Score_Groups.desc')[,DS==dataset])==1 & (attach.big.matrix('Model_Score_Groups.desc')[,DS==invert_ds(dataset)])==1
		i = i[selected_models]; model_score_i = model_score_i[selected_models]
...
#lapply(DS,function(ds) write.csv(Structure_Choice[[ds]],file=paste0('data_out/Structure_Choice.',ds,'.csv')))
write.xlsx(Gene_Choice,file=paste0('data_out/Gene_Choice.xlsx'))
...
#			names.arg=sc$rxn_name,beside=T,las=2,ylab='Mean Model Score Improvement\n(with HMO - without HMO)',legend.text=c('Background','Selection','Intersection'))
	...
	#	error.bar(x,tmp, 1*(t( data.matrix( sc[idx<-!grepl('HMO',sc$rxn_name),c('mean_MS_background.SD0','mean_MS_background.SD1')] ) )))
		...
		barplot( t( data.matrix( sc[,c('enrichment','enrichment_inter')] ) ),names.arg=sc$rxn_name,
...
#				geom_point()+
		...
		grbs =lapply(strs[-5],function(x){
			print(x)
			data_tmp = unique(dfm[grepl(x,dfm$rxn_name)& dfm$variable%in%vars[['prop']][-1] & !grepl('secretor',dfm$ds)& grepl('inter',dfm$variable),])
#			data_tmp = unique(dfm[grepl(x,dfm$rxn_name) & dfm$ds%in%datas[[d_i]] & dfm$variable%in%vars[['prop']][-1],])
			data_tmp$i = gsub(paste0(x,'_'),'',as.character(data_tmp$rxn_name))
			#data_tmp$HMO = gsub('[1-9]|[a-z]|_','',as.character(data_tmp$rxn_name))
			data_tmp_e = unique(dfm[grepl(x,dfm$rxn_name)& dfm$variable%in%vars[['enrich']]& !grepl('secretor',dfm$ds)& grepl('inter',dfm$variable),])
			#data_tmp$sec=ifelse(grepl('secretor',data_tmp$ds),'secretor','all')
#			data_tmp_e = unique(dfm[grepl(x,dfm$rxn_name) & dfm$ds%in%datas[[d_i]] & dfm$variable%in%vars[['enrich']],])
			data_tmp$enrichment = apply( data_tmp , 1 , function(x){
				if(grepl('inter',x[3])){
...
if(cooc){
	...
	lst[['background_coocur_count']] = coocur_par( 'inclusion.rxn.desc',which(rxn),T,cocount,names=rxn_full[rxn],dim_i=1)
	lst[['background_coocur_jaccard']] = coocur_par( 'inclusion.rxn.desc',which(rxn),T,jaccard,names=rxn_full[rxn],dim_i=1)
	...
	for(dataset in DS){
		...
		lst[[paste('select_coocur_count',dataset,sep='_')]] = coocur_par( 'inclusion.rxn.desc',which(rxn),selected_models,cocount,names=rxn_full[rxn],dim_i=1)
		lst[[paste('select_coocur_jaccard',dataset,sep='_')]] = coocur_par( 'inclusion.rxn.desc',which(rxn),selected_models,jaccard,names=rxn_full[rxn],dim_i=1)

		lst[[paste('selectInter_coocur_count',dataset,sep='_')]] = coocur_par( 'inclusion.rxn.desc',which(rxn),selected_models_inter,cocount,names=rxn_full[rxn],dim_i=1)
		lst[[paste('selectInter_coocur_jaccard',dataset,sep='_')]] = coocur_par( 'inclusion.rxn.desc',which(rxn),selected_models_inter,jaccard,names=rxn_full[rxn],dim_i=1)
...
if(cluster){
	...
	for(dataset in DS){
		print(dataset)
		# load inclusion
		inclusion_i = attach.big.matrix('inclusion.rxn.desc')
		# select models
		model_score_i = Model_Scores[,DS==dataset]
		infeasible = is.na(model_score_i)
	...
	tmp=do.call(cbind,lapply( names(out6) , function(xn){
		...
		i=sapply(sort(unique(x$cut)) , function(ci){
			...
			inc_rxns = attach.big.matrix('inclusion.rxn.desc')[idx<-!(grepl('HMO',rxn_names)),sel_i]
...
if(new_sels){	
	...
	inc_rxns = attach.big.matrix('inclusion.rxn.desc')[idx<-!(grepl('HMO',rxn_names)),data12_secretor12]
