function [neighborRxns,neighborGenes,mets] = findUpstreamRxns(model,rxns, asSingleArray, order, commonMets, withComp)
...
for i = 1:numel(rxns)
	...
	metIndex = find(model.S(:,findRxnIDs(model,rxn))==-1);
	...
	for i = 1:length(metIndex)
    	nRxnIndexs{i} = find(model.S(metIndex(i),:)==1);
	end 
