function [model,rxnIDexists] = addReaction(model,rxnName,metaboliteList,stoichCoeffList,revFlag,lowerBound,upperBound,objCoeff,subSystem,printFlag,grRule,geneNameList,systNameList,checkDuplicate)
...
% Reversibility
if (~exist('revFlag','var') || isempty(revFlag))
    if (oldRxnFlag)
...
if (~exist('objCoeff','var') || isempty(objCoeff))
    if (oldRxnFlag)
...
if  isempty(subSystem)
    if (oldRxnFlag) && (isfield(model,'su(nargin < 10)bSystems'))
        subSystem = model.subSystems{rxnID};
    else
        subSystem = '';
    end
end
if ~exist('grRule','var') && (isfield(model,'grRules'))
    if (oldRxnFlag)
